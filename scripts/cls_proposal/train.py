    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if args.calc_sample_loss:
                bce_loss, dice_loss = loss_masks(pred_logits, mask_1024.unsqueeze(1).float())
                loss = bce_loss + dice_loss
            else:
                bce_loss = bce_loss_fn(pred_logits.squeeze(1), mask_1024[:].float())
                dice_loss = dice_loss_fn(pred_logits, mask_1024.unsqueeze(1))
