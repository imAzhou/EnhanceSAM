...
class ImageEncoderViT(nn.Module):
    def __init__(
        self,
        img_size: int = 1024,   # 1024
        patch_size: int = 16,   # 16
        in_chans: int = 3,      # 3
        embed_dim: int = 768,   # 1280
        depth: int = 12,        # 32
        num_heads: int = 12,    # 16
        mlp_ratio: float = 4.0, # 4
        out_chans: int = 256,   # 256
        qkv_bias: bool = True,  # True
        norm_layer: Type[nn.Module] = nn.LayerNorm,
        act_layer: Type[nn.Module] = nn.GELU,
        use_abs_pos: bool = True,   # True
        use_rel_pos: bool = False,  # True
    ...
    def forward(self, x: torch.Tensor, need_inter: bool=False) -> torch.Tensor:
