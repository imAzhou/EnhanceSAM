...
def convert_single_core(proc_id, image_set, categories, source_folder, segmentations_folder, VOID=0):
    ...
    for working_idx, image_info in enumerate(image_set):
        ...
        try:
            original_format = tifffile.imread(os.path.join(source_folder, file_name)).astype(np.uint32)
            # original_format = np.array(Image.open(os.path.join(source_folder, file_name)), dtype=np.uint32)
        except IOError:
            raise KeyError('no prediction tif file for id: {}'.format(image_info['id']))
        ...
        for el in l:
            ...
            isthing = categories[sem]['isthing']
            if isthing == 1:
                bbox = mask2bbox(mask.astype(np.uint8))
            else:
...
def converter(source_folder, images_json_file, 
    ...
    categories_coco = d_coco['categories']
...
python process_datasets/twochannel2coco_panoptic.py \
    --source_folder /x22201018/datasets/MedicalDatasets/MoNuSeg/test/panoptic_seg_anns \
    --images_json_file /x22201018/datasets/MedicalDatasets/MoNuSeg/test/panoptic_anns.json \
    --segmentations_folder /x22201018/datasets/MedicalDatasets/MoNuSeg/test/panoptic_seg_anns_coco \
    --predictions_json_file /x22201018/datasets/MedicalDatasets/MoNuSeg/test/panoptic_anns_coco.json
